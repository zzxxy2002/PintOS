...
void list_apply(struct list* list, void (*func)(struct list_elem* e)) {
  ASSERT(list != NULL);
  struct list_elem* e;
  for (e = list_begin(list); e != list_end(list); e = list_next(e)) {
    func(e);
...
void list_clear(struct list* list, void (*clear_func)(struct list_elem* e)) {
  ...
  ASSERT(list_empty(list)); //list should be empty now
  ASSERT(list != NULL);     //the function itself does not free the list.
}
...
struct list* list_deepCopy(struct list* list, struct list_elem* (*copy_func)(struct list_elem* e)) {
  ASSERT(list != NULL);
  ...
  struct list_elem* e;
  for (e = list_begin(list); e != list_end(list); e = list_next(e)) {
    list_push_back(newList, copy_func(e));
  }
  return newList;
}
